...
# 1. 生成全局调色板
ffmpeg -v warning -i "$INPUT" -vf "fps=12,scale=480:-1:flags=lanczos,palettegen" -y /tmp/palette.png

# 2. 使用调色板生成高质量 GIF
ffmpeg -v warning -i "$INPUT" -i /tmp/palette.png -lavfi "fps=12,scale=480:-1:flags=lanczos [x]; [x][1:v] paletteuse" -y "$OUTPUT"
```
