...
- **断言文档/prompt 的措辞** —— 这些文字要能随时改得更好，断言散文只会把它锁死在
  某一版写法上。曾经有 106 项，结果是每次改文档都得先去改测试伺候它。
...
### 3.03 效果问题的处理流程：先看产物，再谈改动
转换质量的问题（糊、尺寸不对、体积失控）**不许凭直觉改 prompt 或工具说明**。
命令看起来是否合理与产物实际好不好是两回事，只读日志永远得不出结论。

1. 去 `workspace/archive/` 找那次任务的目录（`run.json` 里有用户原话、
   agy 的回复、产物清单）。
2. 拿 `in/` 的原文件和 `out/` 的产物**实测**：`ffprobe` 比分辨率/帧率，
   `ffmpeg -lavfi ssim` 比画质，`ls -l` 比体积。
3. 拿着数字来讨论。**默认结论不是"给 agy 加一条规则"** —— 先弄清是它没看清
   素材、没拿到工具，还是这次本来就没有更好的解。加规则是最后手段，
   因为每加一条都是在替它做判断，而那正是上一版失败的地方。

留痕不在时（`TG_ARCHIVE_ENABLED=0` 或早于配额期限）就用同类素材复现一遍，
**不要跳过测量直接改东西**。

已实测的 GIF 事实，留作参照（源均 2940x1912，fps=12，max_colors=128）：
...
同为 1440 宽，两类素材差 20 倍以上 —— 这就是为什么任何写死的参数都是错的，
也是为什么参数这件事最终整个交给了 agy。

> `core/run_archive.py` 负责留痕与配额，`tests/test_run_archive.py` 守护它。
> 归档目录不受启动清扫影响；回收只按 `.env` 的容量/天数配额进行。

### 3.04 文件任务只做转发，不做预判
这一层反复犯同一个错：**替 agy 做判断**。历代形态都被推翻过——

| 版本 | 预判方式 | 为什么塌 |
|---|---|---|
| 关键词意图分流 | 词表判"处理 vs 问答" | 词表要猜用户怎么说话，转发件 caption 一空就误判 |
| 菜谱按扩展名命中 | 11 份手册内联进 prompt | 手册里写死 `scale=720`，高分屏录屏文字全糊 |
| bpp 分档 | 按码率划三档定宽度 | 把一个常数换成三个常数，中间档连锚点都没有 |
| 素材判定前置调用 | 抽帧让模型先判类型 | 仍是把 agy 本来就会做的事拆出来做一遍 |

现在的形状只有一句：**把文件和用户原话原样交给 agy，它自己动手，产物发回去。**

```
落盘文件 → prompt（工具说明 + 用户原话 + in/out 路径）
        → agy（一次性调用 = 新会话，自己跑命令、自己看报错、自己重试）
        → 扫 out/ → 回传产物 + agy 的文字回复
```

Python 只保留 agy 够不着的部分：文件名收敛（路径进 shell）、产物回收与打包、
Telegram 投递保真、任务留痕。**不解析命令、不执行命令、不回喂重规划、不分流。**

> ⚠️ 想加"预判"之前先看上面那张表。要加规则，得先有 3.03 的实测数字，
> 并说清为什么 agy 自己做不到 —— 而不是因为"这样更可控"。
> 唯一的例外是 `TOOLCHAIN.md`：它说的是**这台机器上有什么**（客观事实），
> 不是**该怎么用**（判断）。这条界线要守住。
